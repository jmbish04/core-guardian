...
export function BacklogDataGridView() {
  ...
  const handleAction = useCallback(
    (action: BacklogRowAction, item: BacklogItem) => {
      setItems((current) => {
        if (action === "remove") {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setItems((current) =>
      normalizeOrder(arrayMove(current, oldIndex, newIndex))
    )
  }
  ...
  return (
    ...
    >
      <Frame variant="default" spacing="sm" className="bg-card! w-full">
        <FrameHeader className="text-foreground flex-row items-center justify-between gap-3">
