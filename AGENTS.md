...
26. **Ollama Cloud is an integrated model source.** `src/backend/guardian/model-catalog.ts` fetches available models from `https://ollama.com/api/tags` (Bearer via `getOllamaApiKey()`, secret `OLLAMA_CLOUD_API_KEY`, already in the Secrets Store — no setup) alongside OpenRouter, AI Pricing Guru, and the scraped Workers AI catalog. Ollama entries land in the unified KV catalog (`model-catalog:latest`) with `source: "ollama"` and null pricing (the list API doesn't expose rates). The AI Router, resolve-model, and recommendations engine all read this catalog, so Ollama models are resolvable by name. Refreshes daily on the cron via `refreshModelCatalog()`; manual trigger `POST /api/guardian/model-catalog/refresh`. **Ownership boundary (agreed cross-session):** per-model token pricing + catalog/discovery live HERE (`model-catalog.ts`); the Ollama RUNTIME provider (`ai-router/ollama.ts`: `/run` model validation, `OLLAMA_KV` cache, usage scraping, `/ollama/*` routes) and the pricing-PLAN watcher (`ai-router/ollama-pricing.ts`, alerts→D1 not DO) are the AI-Router side. `getOllamaApiKey()` is the canonical key accessor — never read `OLLAMA_CLOUD_API_KEY` inline (the generic `resolveKey` in `ai-router/providers.ts` is exempt: it dispatches every provider through `PROVIDER_KEY_BINDING`). ONE fetch of `/api/tags`: `getAuthoritativeOllamaModels()` (in `ai-router/ollama.ts`) owns it + the `OLLAMA_KV` cache; `fetchOllama()` here reads that cache rather than re-fetching.
