...
26. **Browser automation ⇒ BrowserClaw (BrowserOS neo) first.** For any agentic browser task — verifying a signed-in page (§18), driving the app, or firing an authed endpoint through a real session — prefer the **BrowserClaw** MCP (`mcp__BrowserOS_neo__*`); it is tuned for agentic operation. The Chrome `Claude_Browser` MCP stays supported as a fallback — keep both working, don't remove either. The app is **BrowserClaw.app**; if the MCP is unreachable, launch it with `open -a BrowserClaw` and wait for its MCP server (`http://127.0.0.1:9200`) to respond before retrying — do not fall back to the Chrome browser just because BrowserClaw was merely closed.
