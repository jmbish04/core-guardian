...
26. **Ollama Cloud is an integrated model source.** `src/backend/guardian/model-catalog.ts` fetches available models from `https://ollama.com/api/tags` (Bearer `OLLAMA_API_KEY` from the Secrets Store) alongside OpenRouter, AI Pricing Guru, and the scraped Workers AI catalog. Ollama entries land in the unified KV catalog (`model-catalog:latest`) with `source: "ollama"` and null pricing (the list API doesn't expose rates). The AI Router, resolve-model, and recommendations engine all read this catalog, so Ollama models are resolvable by name once the key is set. To add the key: `wrangler secret put OLLAMA_API_KEY`. The catalog refreshes daily on the cron via `refreshModelCatalog()`; the manual trigger is `POST /api/guardian/model-catalog/refresh`.
